...
$sentence=$database->query('SELECT*FROM articulos');
$items=$sentence->fetchAll(PDO::FETCH_OBJ);


$qry = "SELECT SUM(valegr) AS count 
        FROM articulos ";

$suma = $database->query($qry);
$val=$suma->fetch(PDO::FETCH_NUM);
$total=strval($val[0]);

$qry2 = "SELECT SUM(valing) AS count 
        FROM articulos ";

$suma2 = $database->query($qry2);
$val2=$suma2->fetch(PDO::FETCH_NUM);
$total2=strval($val2[0]);
...
<body>
	...
	<center>
		...
		<div style="cursor: pointer;" onclick="location.href='index.php';" class="jogabonito">
	...
	<br><br>
		...
		<table cellpadding="3" class="scrolldown">
			...
			<tr>
				...
				<th><h4 style="color: darkorange;">Dinero Ingresado</h4></th>
				<th><h4 style="color: olivedrab;">Dinero Egresado</h4></th>
				<th><h4 style="color: indianred;">Saldo Actual</h4></th>
		...
		<tbody>
			...
			<tr>
				...
				<td style="color: green;" align="center"><?php echo $item->valegr ?></td>
			
				<td style="color:red;" align="center"><?php echo $item->valing ?></td>
			
				<td align="center" style="color: <?=$color?>;font-weight: bold"><?php echo ($item->valegr-$item->valing) ?></td>
		...
		<br>
		<h1>Saldo Actual en Caja:</h1>
		<h1 style="color:<?php 	if($saldo < 0) {
    			$color = 'darkred';
    		}else{
    			$color='forestgreen';
    		} echo $color ?>">$<?php echo $saldo ?></h1>
		<br>
		...
		<table cellpadding="3" class="scrolldown">
			...
			<tr>
				...
				<th><h4 style="color: darkred;">Eliminar</h4></th>
		...
		<tbody>
			...
			<tr>
				...
				<td align="center"><a style="text-decoration: none;font-size: 20px" href="eliminar.php?id=<?php echo $item->id; ?>">☫</a></td>
			</tr>
		...
		<form method="POST" action="insertar.php" autocomplete="off">
			...
			<td class="hover"><input type="text" class="form-control-lg border-0" name="txtEntradas"></td>
			<td class="hover"><input type="text" class="form-control-lg border-0" name="txtSalidas"></td>
			<td class="hover"><input type="text" class="espacio" name="nou" disabled></td>
